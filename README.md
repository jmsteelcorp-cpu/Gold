...
5. After the first deploy, run `npx prisma db push` and `npm run db:seed` once against your production
   ...
   > **If `db push` errors out on a Neon database**: Neon's connection string with `-pooler` in the
   > hostname is a pooled (PgBouncer) connection, which Prisma's migration engine can occasionally choke
   > on for DDL operations. If that happens, grab the *non-pooled* connection string from your Neon
   > project's Connection Details page (same page, usually a toggle labeled "Pooled connection" — turn
   > it off), run `db push`/`db seed` against that one time, then switch `DATABASE_URL` back to the
   > pooled string for the actual running app (better for serverless connection limits).
